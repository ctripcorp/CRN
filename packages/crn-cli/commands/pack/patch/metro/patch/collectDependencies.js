...
function collectDependencies(ast, options) {
  ...
  const visitor = {
    CallExpression(path, state) {
      ...
      //CRN BEGIN      
      if (
        (state.dependencyCalls.has(name) && !path.scope.getBinding(name)) ||
        name === "lazyRequire"
      ) {
        const result = processRequireCall(path, state);
        const reqNode = result.node;

        if (result === "event.moduleId") {

        } else if (result === "this.__lazy_module_id__") {

        } else {
          visited.add(reqNode);
        }
      } //ORIGINAL: visited.add(processRequireCall(path, state).node);
      //CRN END
    },
...
function processRequireCall(path, state) {
  const name = getModuleNameFromCallArgs(path); 
  //CRN BEGIN
  if (name != null && name.length >= 3 && name[1] === true) {
    return name[2];
  } 
  //CRN END
...
function getModuleNameFromCallArgs(path) {
  ...
  if (node.arguments[0].object) {
    if (
      ...
      node.arguments[0].property &&
      node.arguments[0].property.name === "__lazy_module_id__"
    ) {
      reqFirstName = "this";
      reqSecondName = "__lazy_module_id__";
    }
  }

  const nameExpression = node.arguments[0];

  if (reqSecondName && reqFirstName) {
