/**
 * 1、修改nativeRequire(moduleId),对应native层根据moduleId加载模块
 * 2、moduleId string转number处理
 */
"use strict";
...
function metroRequire(moduleId) {
  ...
  if (typeof moduleId === "string") {
    var tmpModuleId = parseInt(moduleId);

    if (!isNaN(tmpModuleId)) {
...
function loadModuleImplementation(moduleId, module) {
  ...
  if (!module && nativeRequire) {
    ...
    //CRN BEGIN
    nativeRequire(moduleId); //ORIGINAL: nativeRequire(localId, segmentId);
    //CRN END
