...
public class JSDebuggerWebSocketClient extends WebSocketListener {
  ...
  public void injectCRNGlobalObject(String key, String value) {
    ...
    int requestID = mRequestID.getAndIncrement();
    mCallbacks.put(requestID, new JSDebuggerCallback() {
      @Override
      public void onSuccess(@Nullable String response) {
        FLog.d("react", "injectGlobalObject: " + response);
      }

      @Override
      public void onFailure(Throwable cause) {
        FLog.d("react", "injectGlobalObject: ", cause);
      }
    });
    ...
    try {
      ...
      new JsonWriter(sw)
              .beginObject()
              .name("id").value(requestID)
              .name("method").value("injectGlobalObject")
              .name("objectName").value(key)
              .name("objectValue").value(value)
              .endObject().close();
      sendMessage(requestID, sw.toString());
    } catch (IOException e) {
      triggerRequestFailure(requestID, e);
