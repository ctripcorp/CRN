...
void JSIExecutor::registerBundle(
  ...
  void JSIExecutor::CRNLoadModule(uint32_t bundleId, uint32_t moduleId) {
    ...
    try {
      ...
      if (m_moduleIdConfig.size() == 1) { //只有modulePath这个参数了
        jsFileAbsPath = m_moduleIdConfig.at("modulePath") + "/" + key + ".js";
        //          printf("CRN Read js file:%s\n", jsFileAbsPath.c_str());
      }
      else {
        jsFileAbsPath =  m_moduleIdConfig.at(key);
      }
    } catch (...)      {
      std::string errorMsg = "Error: CRN nativeRequire模块ID=[";
      errorMsg += std::to_string(moduleId);
      errorMsg += "]有误。(业务模块ID应>=666666)";
  ...
  std::string JSIExecutor::readCRNJSFileContent(std::string jsFileAbsPath) {
    ...
    struct stat buf;
    int fd = open(jsFileAbsPath.c_str(), O_RDONLY);
    if (fd == -1) {
      printf("open jsfile failed: %s", jsFileAbsPath.c_str());
      return  "";
    }
    fstat(fd, &buf);
    ...
    size_t length = fileLen + 4;
    char *buff = (char *)malloc(length);
    memset(buff, '\0', length);
    size_t realLen = 0;
    
    FILE *file =fopen(jsFileAbsPath.c_str(), "r");
    if (file != nullptr) {
      realLen = fread(buff, 1, fileLen, file);
      fclose(file);
    }
    std::string code = "";
